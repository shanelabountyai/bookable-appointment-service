...
- **A STATE CHANGE is never one edit either** — the same rule a layer up, from the operator review at the Phase 6 close. Anything that changes **when** an appointment happens, **how long** it takes, or **whether it occupies its time** has readers keeping their own copy of the day, and adding a fourth way to change it means finding them all. Phase 6 added three and told none of them: A-018's push moves `startAt` and leaves the running-late delta standing (so every projected chip double-counts it), and A-055 shortens a visit without telling `/staff/opened`, whose entire job is selling freed time — its own backlog row claims the opposite, "for free, because it derives", and that claim is false.
- **A NARROWER LIST IS A NEW FACT** — the Phase 8 close's process note, and the rule below generalised from columns to lists. A-075 added no column: it added `PUSHABLE_STATUSES` ("whose time may this action move") beside `ACTIVE_STATUSES` ("whose time is occupied"), and the moment those stopped being the same set, every reader that had used one as a stand-in for the other was wrong. The proof was inside ONE function — the push's provider axis asked the new list and its chair axis asked the old one, 253 lines apart — and **the compiler cannot see it, because both are lists of the same type.** Its companion: the database can grow an invariant the application cannot read. A-063 added a third exclusion constraint and `errors.ts` still knew two, so nine items shipped while a real refusal reached the desk as a stack trace. The status predicate has a migration test comparing the live SQL against the TypeScript list; **constraint NAMES now have the same test** (A-078 — `OUR_EXCLUSION_CONSTRAINTS` vs `pg_constraint`, set equality, and the push reads that same list to defer), and a test that asserts a raw error string is written from the outside of a bug rather than from the inside of a workflow. **A-078 added a second half to that rule: one violation has more than one ERROR SHAPE.** The same constraint refused at COMMIT rather than at statement end comes back through Prisma with no SQLSTATE anywhere — only the constraint name — so any handler that requires `23P01` is dark on every deferred path, and `push-column.ts` is the only place that defers. Provoke both shapes for real; a mapper tested on one shape is a mapper tested on neither.
- **The readers to grep for keep their own copy of the same fact under a DIFFERENT NAME** — the Phase 7 close's process note, and the sharpest version of the three rules below it. A-069 added a nullable `releasedAt` that cuts an appointment's blocked range, told the two triggers that write `blockedStart`/`blockedEnd`, and stated in its own header comment that the exclusion constraint, the busy set, *the chair holds* and the engine therefore all follow for free. Half true: the chair hold's ENVELOPE follows and its BODY does not, because `bodyStart`/`bodyEnd` are the same fact written from `startAt`/`endAt` under another name — so the grid offered the freed time and the room refused it, which is the offered-then-refused class this repo has now caught three times. **When a new column changes what a range MEANS, do not grep for readers of that column. Grep for everything that stores or derives the same fact under a different name.** And: an item that changes occupancy must have a fixture with a ROOM in it — A-069's test file created no chairs at all, and a one-chair room is the only size that can fail.
