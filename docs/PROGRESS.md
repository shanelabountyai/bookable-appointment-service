...
**Two defects, one cause: every date-relative surface in Phases 6–8 reads a book anchored to a constant twelve weeks in the past.** Measured against a freshly reset and seeded database on 2026-09-02: `unfinished 0`, `opened up 0`, `on today's book 0` — against 227 seeded appointments and **176 rows past and still open**. The seed anchors to `SEED_ANCHOR_DAY = 2026-06-09` and that is *correct*: a `now`-anchored seed makes the DST fixtures exist in March and vanish in July with nothing failing. But it meant nobody had ever seen `/staff/unfinished`, `/staff/opened` or the day grid on a full book — every one of them rendered its empty state on a fresh install. The e2e suite structurally could not say so: `e2e/fixtures.ts` TRUNCATEs and seeds its own rows before every spec, so a feature dormant on a fresh install is dormant in e2e too. This is CLAUDE.md's own "dormant on a fresh install" trap wearing a demo hat.

**Two books, two anchors, side by side.** The fixed book does not move by a day. A second, `now`-anchored one runs nine days either side of today: past days closed out **unevenly and on purpose** (one in four never checked in, one in four checked in and never closed — A-076's opening paragraph is the fixture), a future cancellation so `/staff/opened` has something to sell, and today's last no-show **released** so A-069's cut is visible in a real book. `now` is a parameter on `seedDensity`, never `new Date()` inside it, so a test can freeze the day and assert the shape.

**The two books are kept apart by a ±3-day margin, not by a ±0 one.** A collision is not a crash — it is A-024's exact seeded utilization constant quietly measuring extra appointments, and that assertion is over DEMO_WEEK's whole **ISO week**, not its five open days. The moving window skips any day within three of a fixed fixture, and a test asserts the disjointness rather than trusting the arithmetic.

**Counting cannot see this defect, which is why the assertions are about screens.** The book was full; it was full in June. Four new tests ask what the surfaces *say*: today's book is non-empty, `countUnfinished` is non-zero **and the open rows are not all on the same status** (a screen whose whole point is "she was seen to arrive" vs "she never checked in" demonstrates neither if every row says the same thing), `listOpenedSlots` is non-empty, and the released no-show's `blockedEnd` equals its `releasedAt` — the trigger's cut, not the seed's.

**D-48: the bound was right, its role as a ceiling was wrong.** Past day 22 a row could never be closed, so `dashboard.ts`, `lapsed.ts` and `reliability.ts` stayed permanently wrong about it — and D-46's entire argument is that the reports become right *because* the desk can tell them the truth. Not a new concept: `listUnfinished`/`countUnfinished` already took `lookbackDays`, and A-073's lapsed report already established the shape — a number ON the report rather than a setting, GET so the answer is a URL and no client JavaScript is needed. The badge keeps the 21-day default on purpose: the badge is tonight's errand, the box is the backlog behind it, and the e2e asserts *both* so a later "make them consistent" tidy-up cannot quietly undo the decision.
...
A-082 threaded it through every caller that holds an APPOINTMENT: `reschedule.ts`, `change-services.ts`, the booking re-check. It missed both callers that hold a CLIENT and no appointment yet, **which is exactly what a booking panel is**. `/staff/book/page.tsx` computed `prefillClient` and called `staffSlotsFor` four lines later without it; `booking-panel.tsx` had the chosen client in React state and never read it; `anyProviderTimes` had no client field at all, so the "anyone on Thursday" path could not pass one even if it had wanted to.

**So the offer was STRICTER than the write, on the desk's own screen.** Measured on a two-chair room: 9 slots offered anonymously, 11 with her holder, and the write accepts the withheld ones. What the desk reads is *"every chair is taken then — she is free, the room is not"* about a chair that is hers and empty. **And the workaround is worse than the refusal:** the only way past that sentence is a BOOK-05 override, and by D-30 an override holds **no chair at all** — so one wrongly refused offer becomes one client sitting in a chair the room believes is empty, and the room's model of the day is then wrong for everybody else for the rest of the afternoon.

**What it built.** Four offering surfaces now carry the holder, not three:

- `staffSlotsFor(providerId, serviceIds, day, clientId)` — the provider column, threaded from `page.tsx`'s already-resolved `prefillClient` and from the panel's client state.
- `anyoneTimesFor` → `anyProviderTimes({ …, holderKey })` — "I don't mind who" never meant "I don't mind whose chair". `anyProviderAt` forwards it too, so A-071's *"who else could take her"* recovery is not stricter than the booking it is recovering from.
- `findWalkInOptions` → `walkInOptions({ …, holderKey })` — **not in the item's scope and in scope for the defect**: a named walk-in is usually a client already in a chair for something else, and the stranger at the door is the `null` case. It is the same one-line thread through the same `computeDaySlots`.
- The panel reloads on **client change**, because BOOK-04's order asks for the client LAST — services → who and when → client. Every list on the screen had been computed for nobody. Naming her can only *widen* the answer (sharing relaxes what one holder may do and nothing else), so the reload keeps the selection: `anchorAt` re-selects the same instant, matched exactly rather than "the first at or after", which is right for a gap the desk tapped and would quietly move her here.

**The reload cost two defects on the way in, and the e2e gate caught both.** Worth recording because neither is about the room:

- **A REFUSED CHIP THAT IS SELECTED IS STATE.** The first version re-selected through the gap anchor's "first BOOKABLE time at or after", so naming the client after tapping a refused time silently moved the selection to a later bookable one — disarming A-042's override box and arming a booking for a time the desk had not chosen. The instant the desk tapped is kept exactly, refused or not; a refused-but-selected chip is the only way to reach BOOK-05 from a screen.
- **"NO NAME" IS A SENTINEL, NOT A NULL.** `NO_NAME` is `{ id: '', … }` — the form's way of saying *booked with nobody* — so an object comparison read tapping it as a change of holder and reloaded for nothing, throwing away a typed 18:00 (`loadFor` clears the composed times) and leaving the Book button inert. One `holderOf(choice) => choice?.id || null` normaliser now decides both the comparison and what the server is told, because an empty key already IS the strict question.

Both failures came from specs written for other items, on flows this item never mentions — the e2e sweep's actual value, and the reason the gate is four commands rather than three.

**The test is an AGREEMENT test, which is the whole point.** `packages/db/booking/holder-agreement.test.ts` — 5 tests. The core one takes every candidate whose fate the ROOM decided (offered, or refused with exactly `no-resource-free`, which `computeSlotsIn` only ever attaches to candidates that already passed the engine) and asserts in ONE assertion that *offered* equals *the chooser would seat it*, over a two-chair room where the two answers genuinely differ. Two separate assertions — "the offer says X", "the write says Y" — both pass while X and Y disagree, and that is precisely how the read model and the chooser drifted apart for fifty items. The test's own envelope arithmetic is checked once against the engine's own numbers, so the loop cannot be two copies of one mistake. Verified by reverting the two `holderKey` threads: 2 of the 5 fail.
...
## The design pass, scoped — and A-088, the token layer

**Scoping.** The design brief's §8 is eight deliverables and is not one session. Sequenced to unblock the shell first, which is the order D-49 already named (§8.1 tokens → §8.2 primitives → §8.4 the shell): **A-088** the token layer and type scale, **A-089** only the primitives the shell uses, then **A-085** itself, whose dependency changes from "the design pass" to A-089. The rest — `AppointmentChip`/day grid (A-090), `/staff/opened` (A-091), lapsed (A-092), the print sheet (A-093), mobile public booking (A-094) — are restyle items behind the shell. No new D-number: the order is what D-49 said, only written down as rows.
...
**What it built.** Semantic tokens on `:root` — ground (page / raised / sunken / inverted), ink (primary / secondary / muted / inverted / disabled), line (hairline / control / strong), four intents × ink+line+fill, focus ring, radius, one shadow, one motion pair — re-declared under `prefers-color-scheme: dark` and mapped into `@theme inline`. Five type roles, body at **14px** because that is what the product actually is.

**Three places the brief's list was cut, each for the same reason — a token with no caller has no state anybody has checked.** `overlay` (there is nothing that overlays: the desk switcher is a native `<details>`, and the product has no popover, modal or menu). `neutral` as a fifth intent (it is `--ink-secondary` on `--ground-raised` inside `--line-hairline` — a second name for three facts). `body-sm` and `label` as sizes (`label` is `caption` at medium weight in secondary ink — a role, not a measurement). Each cut is written into the file beside where the token would have been, so the next person meets the reasoning rather than the absence.

**The `var()` indirection is the load-bearing decision, and it was verified in the built CSS rather than assumed.** Because `@theme inline` maps to `var(--ground-raised)` instead of copying a hex, the emitted utility is `.bg-ground-raised{background-color:var(--ground-raised)}` — it resolves at use time and flips with the scheme by itself. **That retires the `dark:` twin**, which the audit counted 132 of in one class alone. Checked by building with a probe class and grepping the output, because the whole of A-089 is built on the claim.

**Two values deliberately differ from what the app renders today, and both are the same defect the public palette already had.** `--ink-muted` is zinc-600, not the zinc-500 used in 110 places: #71717a measures **4.83:1** on the page ground and **4.40:1** on the sunken one, so it passes as body text on white and fails inside the very panels muted text lives in. `--line-control` is zinc-500, not the zinc-300/zinc-400 on 133 borders, which measure **1.48:1** and **2.56:1** against the 3:1 bar for a boundary that identifies a control. `--color-muted-ink`'s existing header comment tells the identical story from the marketing pages — one token darkened until it passes on the DARKER ground is the only version that cannot regress halfway through a screen.

**The gap the audit found: three explicit focus styles in the whole staff app**, against §4's "every interactive control is keyboard operable with a visible focus ring". Fixed as one `:focus-visible` rule rather than a `Button` primitive, because a link, a `<summary>`, a native date input and a `tabIndex={0}` scroll region are all keyboard stops and none of them is a Button — one rule where every caller already routes, instead of a prop 200 controls each have to remember.

**Print inherits the fix, because it is the same bug A-062 found.** The `@media print` block now drives every ground to white and every ink — intents included — to black. A sheet printed from a tablet in dark mode otherwise carries #ededed onto paper the printer never inks. §4's "never colour alone" is what makes flattening the intents lossless: the words survive.

**The contrast results are COMPUTED, not stated** (`packages/design/tokens.test.ts`, 29 assertions). §5.1 asked for a stated result per token; a number in a comment is right the day it is written and silently wrong the first time a hex moves — which is exactly how the public palette shipped two tokens that passed axe on half the marketing pages. The test parses the shipped CSS (not a generator's input — the CSS is what ships, and a generated file would leave the shipped one un-asserted) and checks every ink on every ground it can land on, every intent ink against its own fill, and the control border and focus ring at the 3:1 graphical bar. It complements axe rather than repeating it: axe measures what a page paints and cannot see a token no screen has adopted yet.

**Verified by mutation, three ways.** Restoring today's zinc-500 muted ink fails at `4.396 < 4.5` on the sunken ground. Restoring today's zinc-400 control border fails at `2.563 < 3`. And **deleting one dark intent declaration** — the silent case, where a token defined only in light inherits its light value into dark — fails at `2.792 < 4.5` on the dark page ground. A contrast suite that has only ever seen passing values has not been tested.

**Left behind.** Nothing uses the tokens yet; A-089 is the first consumer, and the 133 hand-written borders and 132 `dark:` twins are retired screen by screen behind it, never in one sweep — a repo-wide find-and-replace across surfaces axe currently passes is a way to break several at once. `--ink-disabled` carries no assertion on purpose (WCAG 1.4.3 exempts inactive controls) and `--line-hairline` carries none either, because nothing is identified by it; if either ever becomes the only signal for something, it needs a bar and a row in that test.
...
**The subset is the item, not a shortcut.** §5.3 lists eighteen primitives; this built six. Select, Textarea, Checkbox, Radio, Card, Table, InlineAlert, Sheet, Toast and Skeleton wait for the screen that needs them, because **a primitive with no caller has no state matrix anybody has checked** — and this repo has now found six defects that were invisible from inside the item that introduced them. The same argument applies to this item, which is why `/staff/design` exists at all: the gallery is what gives six brand-new components a caller, an axe run and a keyboard before A-085 puts them on every staff screen.

**`Field`'s children is a function, and that is the whole design.** A field's job is wiring: `htmlFor`, `id`, `aria-describedby` pointing at the hint *and* the error, `aria-invalid` when there is one. Those ids are **one fact**, and every other way of building this writes the fact down twice — a caller passing `id` to both `<Field>` and `<Input>`, or a `Field` deriving `${id}-hint` while `Input` independently derives the same string. Two copies of one fact under two names is the defect class this project keeps finding one layer down (A-086's status pair, A-069's `bodyStart`, A-078's constraint names). The render prop computes the ids once and hands them to the control, and the type signature will not let a caller forget to spread them. `cloneElement` hides the same wiring and silently does nothing the day a child is wrapped; `useId` and context both need `'use client'`, and every form in this product renders on the server.

**The error slot's rule is about `undefined`, not about truthiness, and the reason is ARIA.** A live region must exist *before* the text lands in it — a `<p aria-live="polite">` that appears at the same moment as its own error announces nothing. So `error` omitted entirely renders no element (a search box cannot fail, and an always-present empty region reserves a line for an announcement that never comes), while `error={state.error ?? ''}` renders it always, empty at rest. The reserved line is also what stops the form jumping when the error arrives, which §4 forbids.

**`pending` IS `disabled`, not a look beside it.** Every mutating control in this app is a `useActionState` form (§5.3), and a pending button that is still clickable is a double submit — in a product whose entire correctness story is not writing the same appointment twice. It also sets `aria-busy`, so the state reaches the accessibility tree rather than living only in the caller's swapped label.

**`Tabs` are links and will never be `role="tablist"`.** `/staff/day?provider=…` is a URL the desk keeps open on a second tab and prints from. The ARIA tab pattern would claim keyboard behaviour this is not and would tell a screen reader the panel is on the same page. `<nav>` + `aria-current="page"` is what §4 asks for and what the day view already did by hand.

**The current tab carries a print override even though its only caller is `print:hidden`.** A-088 draws selection by inverting; a black chip with white text is precisely what §4 means by "anything that only works because of a background colour is invisible on paper", because browsers decline to print the background and leave white text on white. On paper the current tab is bold and underlined instead. `app/staff/day/page.tsx:91` happens to hide the whole toolbar when printing today — **a primitive that is only correct because of where its one caller sits is a defect waiting for the second caller**, and A-093 is that caller.

**`Badge` has no accessible name of its own, on purpose.** It is a number; the control around it is the noun. `<LinkButton>Opened up <Badge>3</Badge></LinkButton>` reads as "Opened up 3, link", and a badge that drifts outside its control announces a bare "3" — §4's never-colour-alone rule wearing a different hat, because a count with no noun is not information. The e2e spec asserts the *composed* name (`getByRole('link', { name: 'Opened up 3' })`), which is the only assertion that can see the badge escape.

**The one genuine visual change: desk controls are now 44px tall, and it is measured.** §4's "primary desk targets are ≥44px on the tablet" is a constraint, not a preference, and the same button without that minimum measures **40** — verified by removing it, at which point the spec fails on `40 < 44`. `size="compact"` is the exception the brief allows for dense rows and makes no touch-target claim — the name is the warning. The assertion is `boundingBox().height`, not a grep for `min-h-11`: a class-string test passes the day somebody adds a `py-1` override that wins.

**No focus styling in `Button`.** A-088 draws the ring once on `:focus-visible` for every keyboard stop, and the e2e spec proves it by tabbing — the first stop on a staff page is the desk switcher's `<summary>`, which is not a Button and is exactly why the rule does not live in one.

**The guard: `packages/design/primitives.test.ts`.** No raw palette utility, no `dark:` variant and no hex anywhere under `components/ui`, with comments stripped first so the guard does not fire on its own explanation of the palette it replaced. A `dark:` in a primitive means somebody went around the tokens rather than through them — the value stops flipping with the scheme, stops being covered by `tokens.test.ts`'s contrast assertions, and stops following the print overrides. The directory is globbed, not listed, so a primitive added by a later item is covered without anybody remembering. Verified by mutation: putting `bg-zinc-100 dark:text-zinc-400` back into `Badge` fails two assertions.

**Adopted on two screens, chosen because they already run axe.** `/staff/clients`'s search form (`Field` + `Input` + `Button` + two `EmptyState`s) and `/staff/day`'s view nav (`Tabs`). Both are covered by existing e2e axe runs, so adoption is proven by a suite that was already watching rather than by a new assertion written alongside the change.

**Left behind.** `Badge` has two intents, not five — the other three A-088 tokens get a variant when a caller needs one. `Badge` and `EmptyState`'s only callers until A-085 are the gallery and the clients page respectively. The rest of the staff app still hand-writes zinc on 200-odd controls; it retires screen by screen behind A-085/A-090/A-091, never in one sweep. `/staff/design` is deliberately unlinked — it is a workbench, and A-090 onwards should add their compositions to it rather than inventing a second one.
...
**D-50 answers the three things §5.5 left open** — `/staff`'s new job, how the shell packs at tablet-landscape, and whether a door may hide at zero. The reasoning is in the decisions file; the third is the one worth repeating here.

**The door is permanent and the badge hides at zero, and that settles a conflict rather than picking a side.** A-076 hid the whole *Still open* link at zero, because a permanent link to an empty list stops being read. D-49 then wrote down what it cost: Phase 8's headline screen had exactly ONE inbound door, so a desk that had never had an unfinished appointment could not know the screen existed. Both are right about different things, and only a shell can hold both — the **door** is navigation and is always there; the **number** is an errand and appears only when there is one. A-076's worry was about the *number* becoming furniture, and it does not. `day-grid.spec.ts`'s "the tab is not there when nothing is open" is inverted on purpose and now asserts `{ name: 'Still open', exact: true }`: a link named exactly that is a link with no badge on it.

**The three counts are OWNED by the layout, and that is load-bearing rather than tidy.** §5.5 requires all three visible from anywhere, and `/staff/day` used to compute two of them itself. `listOpenedSlots` is not a `COUNT(*)` — it re-derives whether each freed span is *still* empty, per candidate. Any cheaper number in the badge would be a second, weaker answer to the chooser's question, which is checkpoint 6's defect exactly, and a badge reading 3 above a screen showing 1 is a badge the desk stops trusting. So the shell asks the real question once and the day page no longer asks it at all. `shell.spec.ts` asserts the negative directly: no `Opened up` or `Still open` link inside `<main>`.

**Setup is `requireStaff`, not `requireOwner`, and the tier is drawn by position.** Every route it indexes is `requireStaff` today; `requireOwner` redirects to it, so guarding it would loop; and quietly removing a stylist's access to Services behind a navigation change is not a navigation change (D-36 — the route is the control, not the link). Only Dashboard stays owner-only, exactly as `/staff` had it.

**`'use client'` for exactly one reason: `usePathname`.** A server layout cannot know which route is rendering, and §4 wants `aria-current="page"` on the selected item. Nothing else in the nav is stateful — the counts arrive as props from the layout, where the queries belong, and the search box is a plain GET form that works with JavaScript off. The section match is a prefix so `/staff/dashboard/lapsed` still marks Dashboard, with `/staff` compared exactly so Setup is not current on every screen in the product; the spec asserts all three.

**The test that matters most is the last one.** It starts on the day grid, types "Kerr" into the shell, and lands on her record — §5.5's headline gap, which the operator review named twice: *"It's Mrs Kerr, can I move Thursday"* was day grid → `/staff` → Clients → search, while she waited.

**`/staff/book` is deliberately absent from the nav.** The item is about routes that were hard to reach; booking already has three doors (Walk-in, Anyone, and every empty slot on the grid).

**What left the day toolbar, and the one thing that was renamed instead.** *Opened up* and *Still open* went to the shell with their counts. *Call-down* went too — the shell's link is the identical URL, so it was a pure duplicate. *Conflicts* was **renamed to "Conflicts on this day"** rather than deleted: `/staff/conflicts` defaults to TODAY when given no `?day=`, so the shell's link and the toolbar's genuinely point at different days, and two links with the same name pointing at different days on one page is a worse screen than either. The longer name is the honest one. Walk-in, Anyone and Print sheet stay, because they carry `?day=` and mean nothing without it.

**Three specs failed for the same reason, and it is the item's own lesson pointed back at the suite.** Each had encoded WHERE a fact lived rather than the fact. A-089's focus test asserted the ring was on the *second tab stop* — true until the shell inserted eight; it now tabs until it reaches the Button, which additionally proves keyboard reachability. `messages.spec.ts` asserted the literal string `Messages — 1 not sent`, which was `/staff`'s own rendering of a count that is now a badge. And the sharpest one: `settings.spec.ts` disabled HTML validation with `document.querySelector('form')` — *"the first form in the document"* — which was the providers form right up until the shell put a client search box above every staff screen. It then silently marked the WRONG form, `required` blocked the submit, and the server-side check the test exists for never ran. Reached through the field's own `.form` now: a locator that names what it wants cannot drift like that. While in `messages.spec.ts` a fourth thing turned up unprompted — a sibling assertion missing `exact: true`, so it passed even when a badge appeared, which is the single thing it exists to rule out.

**Left behind.** Three queries now run on every staff page render, one of them per-candidate over a fortnight; marked with a `ponytail:` comment naming the ceiling and the upgrade path (a cached count still derived from `listOpenedSlots`, never a cheaper predicate). The `<select>` in the desk switcher is still hand-written — §5.3 defers a `Select` primitive until a second caller exists, and it is still the only one. A-087, demo checkpoint 7, is what walks the product through this shell on a seeded book.
...
**One defect, found twice in one afternoon, in two unrelated files, for the same reason: every axe run in this suite has looked at an empty screen.** Checkpoints 1–6 each found something sitting inside an item already ✅ and invisible from within it; checkpoint 6 needed a busy room to see anything at all. This one needed a **worked** book — a day the desk has been through, rather than a day with an appointment on it.

**The finding.** `/staff/day` on the seeded book fails WCAG AA on every chip somebody has closed out, in **both** colour schemes:
...
Three decisions, each defensible alone, and the product of them is the defect. `completed` paints a darker ground (`bg-zinc-100`). It also paints a lighter ink (`text-zinc-600`) — 7.0:1 there, correct. And then the chip's detail line multiplied that ink by 0.8.

**A-088's contrast guarantee is a COMPUTED one, and an `opacity-*` is the same fact written a second time under another name.** `tokens.test.ts` reads each token and asserts it against its own ground at its real bar, precisely so a contrast number in a comment cannot rot. An opacity changes the colour the eye receives without changing the token, so every contrast assertion in the repo still passes, and the colour it renders exists only in the compositor — where nothing was looking. This project's most-found defect shape (A-069's `bodyStart`, A-078's constraint names, A-086's status pair), one layer down.

**Why the suite could not see it, and this is the transferable half.** `day-grid.spec.ts` runs axe on that exact page and is green. It seeds ONE appointment, and it is `booked`, whose ground is `bg-white` — where the same 0.8 lands on **4.66:1** and passes by a hundredth of a point. The violation needs a chip somebody has closed out, which is what an evening's book is made of and what no fixture had ever rendered.

**Then the same rule again, ten minutes later, in a different file.** `/staff/clients/[id]`'s "Upcoming" and "Past" headings are `text-zinc-400` — **2.62:1 on white**, a live failure in the default scheme. `clients.spec.ts`'s axe test opens Ada Chen, a client it creates and never books, and **neither heading renders until she has a future visit** (`Past` is guarded on `upcoming.length > 0`). The axe run measured the chrome.

**And the half of the palette nothing had ever rendered.** Playwright's default scheme is light and no spec had ever changed it, so on a palette built to flip with `prefers-color-scheme` half of it had never been measured: 124 nodes on the client record in dark, 2 on the appointment detail, 2 on the dashboard drill-down. Those are the raw-zinc debt A-090 onwards retires — **one value repeated**, `text-zinc-500`, which is 4.6:1 on white and **4.1:1** on `#0a0a0a`. Not this item's defect; this item's business is that nobody knew the number because nothing had ever asked.

**Two things the dark assertion cost on the way in.** `emulateMedia` alone is not a scheme change, it is a scheme change **in flight**: every control is mid-`transition-colors` (A-089's primitives animate on purpose) and axe sampled the blend — 583 nodes of colours like `#b4b4b4 on #5d5d5f` that exist for 150 ms and belong to neither scheme. It reloads now. And the one real dark failure it then found was `<p>Not today.</p>`, which renders only when the desk is looking at a day that is not today — **my own read-only sweep had called `/staff/day` dark-clean because every page it loaded was today's.** The finding caught the walk writing it. So the sweep is all seven of the day surfaces' bare `text-zinc-500` uses, not the one the fixture happened to render.

**What it built.** `opacity-80` off the chip's detail line and its visit note (weight already separates them from the title; the ✎ glyph already separates the note from the pinned one), off `column-controls.tsx`'s struck-through scheduled time, and off `people-list.tsx`'s inactive row — which dimmed a person's NAME with it and says "· off the roster" in words regardless. `packages/design/opacity.test.ts` forbids unprefixed `opacity-*` anywhere under `app/staff`, directory walked rather than listed, with `disabled:`/`hover:` allowed on WCAG's own exemption (1.4.3 does not apply to disabled controls); verified by mutation. `day-grid.spec.ts`'s axe fixture now seeds one chip of every status that repaints the chip, asserts the detail line is on the page so the fixture cannot go quiet, and measures **both schemes**. `clients.spec.ts`'s gives her a visit behind and a visit ahead and asserts both headings first.

**Two doors the walk found shut, both one line.** Her name on the appointment detail is now the link to her record — the only one there had been was the **reliability flag**, which renders only when she has a black mark, so the client with nothing on her record had no door at all and the way through was to retype a name the screen was already showing. And `/staff/dashboard/appointments` reached with no week said *"0 appointments · Nothing matches this filter"* on a full book, which reads as a salon with no appointments in it; it says *"Pick a week from the dashboard."* now, which is what `overruled` next door has always said.

**Walked and found clean, which is worth recording.** *The shell (A-085):* twenty-five staff routes, all 200, the shell on every one, no console or page error anywhere; the "Still open" badge equals the list exactly, because both are one `where`. *The room (A-082/A-083):* 201 offers taken and booked with nothing else running, **0 refused**, across two fuzz rounds and a staggering pass that took the book from 2 chairs in use to 4 — and, in the other direction, 1,288 anonymous-vs-holder comparisons of which **3 widened, 0 narrowed, and all 3 widened slots were accepted by the write**. Three real add-ons a desk can sell that the anonymous question refuses.

**Scene 3 could not be walked at all, and that is a finding.** The demo install has **zero waitlist entries, zero call marks and zero call-down attempts**, so `/staff/opened` → "Who wants this slot?" → *"Nobody on the waitlist fits this one."* The whole sell-the-freed-slot loop is dark on a fresh install, on a book with 453 appointments in it — A-081's own trap, one layer down from where A-081 closed it. Two more shapes of the same gap: the `now`-anchored book fills only Dana and Priya, so **Marcus and Tess have no future appointments at all** and two of the four columns are empty from today onwards; and consequently the room never binds — on every day of the book the number of chairs in use is exactly the number of stylists working, and a read-only sweep of the future book found **21,184 offers and not one candidate refused for `no-resource-free`**. Not fixed here: `seedDensity` is pinned by an every-column idempotence test and by A-024's exact utilization constant, so widening it is an item. **A-095.**

**Left behind.** A-095 (the demo book's dark corners) and A-096 (dark-scheme axe across the rest of the staff app, with the measured node counts attached). `/staff/unfinished` is oldest-first, so the six-o'clock errand is at the bottom of the list — recorded, not changed on a walk. Every staff page costs about **570 ms**, uniformly, including the ones with almost nothing of their own: that is the ceiling A-085's `ponytail:` comment named, now with a number against it. The shell at tablet landscape is 113 px of nav and 150 px of chrome before the grid starts, with no horizontal overflow at 768, 1024 or 1440.
...
**THE FINDING, and it was on the home screen: the day grid's columns did not share a vertical axis.** Each column laid out its own heading, then its own controls, and only then its positioned box — so minute zero sat wherever that column's chrome happened to end, and **the chrome's height depends on the data.** Measured on a seeded Tuesday with nothing running late:

```
gutter 380 | Dana 480 | Priya 480 | Marcus 438 | Tess 438
```

The hour labels were **100px — 67 minutes — above the rows they label**. Marcus and Tess were drawn **42px (28 minutes) above** their two colleagues, because a column with nothing left in it renders no "push the column" control and its chrome is shorter. A column running late grows a whole ring-round panel and drops further still; a stylist who is OFF renders no controls at all and rises.

*"Who is free at two?"* is a question you answer by reading **across** a day grid. It was off by half an hour, differently per column, according to how busy each stylist was — which is the one variable that makes the question worth asking.

**The fix is structural, not a measurement anybody has to maintain.** One grid, two rows, every column a `subgrid` spanning both: row one takes the height of the tallest chrome, so every row-two cell starts on the same pixel, gutter included. No JavaScript, nothing to keep in step, and a column that grows a new control can no longer shift its own minute zero because it no longer owns it. `RoomStrip` had the identical defect for the identical reason — a chair marked *out of service* carries a second phrase in its heading, which wraps — and got the identical fix.

**Why nothing saw it.** Every fixture in the suite had one provider, or four with identical chrome. `day-grid.spec.ts` now sets a running-late delta on one column so its chrome genuinely differs from the three beside it, then asserts that every `data-day-axis` element starts on the same pixel — with a guard that the count is greater than two, because an alignment test over one element is vacuously true and that is how this kind of test dies quietly. Verified by mutation: reverting to the flex layout fails it, naming the four offsets.

**The second defect: the projection stopped at the wrong status.** `view-model.ts` hand-typed `status === 'booked'` — twice, once for the chip and once for its accessible name — which is CLAUDE.md's first structural rule broken in the narrowest possible way: **a status list of ONE, narrower than both of its neighbours.** A running-late column already had two derived lists (`PUSHABLE_STATUSES`, "whose time may a push move"; `STILL_ON_THEIR_WAY_STATUSES`, "who is worth ringing"), and the chip's answer was neither. So a CONFIRMED client in a column forty minutes behind sat on the desk's own ring-round as *"booked 14:00, likely 14:40"* while her chip three inches away on the same screen showed 14:00 and no projection at all — two answers to one question, in one glance. `AWAITING_START_STATUSES` is the third list and the three now nest, each boundary a separate operational fact: **ring her ⊂ project a later start onto her ⊂ a push may move her.** `checked_in` is the first boundary (in the building, so nobody rings her, and the desk still tells her when she goes in); `in_progress` is the second (in the chair, so the push still moves the row and a projected START is not late, it is wrong). Pinned by `status.test.ts`, and by an e2e that puts three statuses in one late column.

**The third: four of the eight statuses were told apart by colour alone**, and two of those four — `checked_in` and `in_progress` — by two adjacent tints of the same hue. §4 is not ambiguous about this, and the chip's defence was that the status word is in the accessible name, which helps a screen reader and does nothing for the colour-blind stylist reading the tablet. Every non-default status now carries a WORD on the chip, `shrink-0` so the client's name truncates around it: a column narrow enough to lose text should lose the tail of a surname, not the fact that this one never turned up. `booked` carries none on purpose and is the only one — it is most of every column, and a marker on all of them is a marker on none.

**The two times, which §4 calls the risk.** Separated by three signals at once — their own lines, an arrow, and the word "likely" — and in the accessible name by naming both: *"booked for 14:00, likely to start 14:40"*, never four bare clock readings in one sentence. Her confirmation still says 14:00 and so does the chip.

**Every line inside the chip lost its colour, and that is a correctness change rather than taste.** The pinned note, the reliability flag and the projected time were amber-on-whatever-the-status-painted. `tokens.test.ts` asserts an intent ink against the three grounds and against ITS OWN fill — amber on emerald was asserted nowhere, which is demo checkpoint 7's defect exactly, one item later. They are the chip's own ink plus a glyph plus weight now, and `tokens.test.ts` gained the pair the chip actually renders: ordinary ink on each intent fill, which nothing had measured either.

**Left behind.** `checked_in` and `in_progress` share one fill and are told apart by their words; a fifth intent would be the alternative and §5.1 has four on purpose. The gallery draws no live status button — the fixtures carry no `appointmentId`, which is the data saying so rather than a prop. `/staff/design` is still deliberately unlinked. A-091 brings `FreedSlotRow` and `CallMarkButtons` to the same page.
...
**THE FINDING: §4's 44px desk target, on the control the desk taps more than any other, was 26px.** `px-2 py-1 text-xs` — a 16px line box, 4px of padding either side, two 1px borders. §4 is explicit that this is a correctness rule and not a preference ("Primary desk targets are ≥44px on the tablet. The grid chip is the exception and is compensated by a detail panel"), and this control is not the exception; it is the one §5.4.9 calls the most-reused interactive control in the product. **And the harm of a mis-tap here is not a wasted tap.** The screen is shared, and marking the WRONG client as rung silently skips her — which is the exact harm the undo beside it exists to reverse, and the undo was an underlined `text-xs` link, *smaller than the thing it undoes*. Every button is now `Button` at its default `md`, 44px, undo included, and `design-system.spec.ts` MEASURES it with a count guard rather than grepping for a class.

**Why nothing could see it, and this one is structural rather than an oversight.** Every spec in the suite drives these buttons by accessible NAME, which did not change. Nothing measured them. And axe cannot help: 44px is WCAG **2.5.5, which is AAA**; the AA rule is 2.2's **2.5.8 at 24px**, which 26px passes and which CI does not run anyway — the four tags are `wcag2a, wcag2aa, wcag21a, wcag21aa`. **The bar the design brief sets is one no tool in the gate can see**, so the only thing that can hold it is an assertion that measures a rendered box.

**The second: the pressed state was not in the accessibility tree at all, and both copies carried a comment explaining why it could not be.** *"`aria-pressed` is not available on a submit button that is also the form's payload."* That is not true — a `<button type="submit">` has role `button`, and role `button` supports `aria-pressed`. So a screen reader met four identically-shaped buttons with no indication of which one stands, and the pressed state was an inverted ground and nothing else: **colour alone (§4) in the accessibility tree as well as on the screen.** A comment that reasons its way to a gap and then writes the gap down as a constraint is worse than no comment, because it is the thing the next person reads instead of checking.

**The third, and it is why the first two were each wrong twice: it was never one component.** A-061 wrote it for the call-down, A-072 wrote it AGAIN for the waitlist — four outcomes instead of two, and a copy of the same eleven-line `button()` style function — and A-073 wrapped A-072's for the lapsed list, correctly noting that a third copy "would be the third shape for one idea" while leaving the second one standing. §5.4.9 had said *one component, three surfaces* from the start. The three things that actually differ are the WORDS (two or four), the form's hidden payload, and the undo's label, so those are the parameters and there is nothing else.

**The fourth: five sentences asserted a gender the record does not hold.** "Mrs Hall dropped **her** Colour", "the rest of **her** time was put back", "**She** took it" on the lapsed report, "**She** never came, and 45 minutes of **her** slot are still blocked" on the release panel, and "Cleared — **she** has not been asked." The `Client` record has no gender field to consult, the salon has male clients, and these are read down the phone. All five now name the client and nothing else.

**Design, and each of these is length rather than taste.** The freed row's reason shared a line with the `tel:` link, so the sentence that distinguishes the five kinds trailed off into a phone number that was not a target — two lines now, and the number is 44px. The marks were `join(' · ')`ed into one line, which §8.6's own composition breaks the moment there are two of them — a labelled list now. And `took_it` read exactly like `no answer`: it is the one mark that means STOP RINGING before you dial, so it carries the positive ink as well as its word.

**Left behind.** `components/client-flag.tsx`, `client-picker.tsx` and `date-jump.tsx` still render raw zinc — `primitives.test.ts` globs `components/ui` only, and widening it today would simply fail; §5.4.6/§5.4.7 are their items. The `tel:` links on the waitlist, lapsed and call-down rows are still inline `text-sm`; only the freed row's got a target. The gallery's call-mark rows carry an EMPTY hidden payload so a press on the workbench cannot write a mark — the data saying the row is not real, A-090's rule.
...
**THE FINDING: on the one screen in the product that is nothing but phone calls, the thing you tap to make the call was 16 pixels tall and 3.9 pixels from a link that navigates away.** Measured at thirty rows on a 1024×768 tablet before anything was changed:

```
Client Number 0   x=105  w=109  h=16   → /staff/clients/<id>
5125551029        x=218  w=78   h=16   → tel:
list 5044px tall = 6.6 screens; row 134px
```

§4's bar is 44px and it is a correctness rule, not a preference. **The harm of the mis-tap is not a wasted tap:** the neighbour is the client record, so a fat finger leaves a 6.6-screen list and comes back to the top of it with no memory of which of the thirty rows had been reached — on the only screen in the product a person works *down*, over more than one sitting, which is the whole reason §8.6a exists as its own deliverable.

**Why A-091 fixing this exact defect one item earlier did not fix it, and why the note it left was itself the bug.** A-091 gave `FreedSlotRow`'s copy `min-h-11` and wrote down what it had not done: *"the `tel:` links on the waitlist, lapsed and call-down rows are still inline `text-sm`; only the freed row's got a target."* Three. **A grep says eight** — those three plus the appointment detail, the stranded list on Providers, the conflicts list, the day column's ring-round and `/staff/opened` itself. The list of known copies was written from memory, which is the same defect one level up: nobody counted, so seven stayed at 16px, including the ring-round on the day screen, which is the one the desk uses while somebody is late. One component now, eight callers, and a guard that walks `app/staff/` rather than a note that has to be remembered — plus a caller-count assertion, because a rule pointing at a component nothing uses passes forever.

**And nothing in the gate could see it, for the second item running.** 44px is WCAG 2.5.5, which is AAA; the AA rule is 2.2's 2.5.8 at 24px, and axe's `target-size` check is not in the four tags CI asserts on anyway. The bar the brief sets is one no tool in the suite can check, so the height is measured against a rendered box at the FIRST and LAST row of the thirty — the previous defect was a target that only survived where somebody had happened to look.

**The href was broken as well as small.** Every staff copy interpolated the stored string straight into `tel:`, so a number typed "(512) 555-0188" dialled nothing. The public site has stripped it since the marketing pages were built (`telHref`); the staff app — where the calls are actually made — never did.

**The second: the report was failing AA in dark, and its own axe test could not see it.** `text-zinc-500` on the back link is 4.6:1 on white and **4.1:1 on `#0a0a0a`** — checkpoint 7's value, this page's turn. The spec had never run axe in dark, and it seeded ONE client with NO call mark, so the mark line — the half of the row that only exists once somebody has started working the list, and the only place this screen uses an intent ink — had never been rendered under axe at all. It now runs both schemes over a list with a fresh mark and a stale one, reloading between them (`emulateMedia` on a live page samples every control mid-`transition-colors`), and asserts the states are on the screen before axe looks.

**The third: 150 buttons drawn from a vocabulary of five words, and none of them said who it was about.** Thirty rows × five call-mark buttons: a screen reader hears "Left a message, button" thirty times over. A-091 put the pressed STATE into the accessibility tree one item ago; the SUBJECT was still missing, on all three list surfaces. One optional `about` on `CallMarkButtons`, passed by the lapsed report, the waitlist matcher and the call-down; the visible label stays the bare word, because the four outcomes need the width on a tablet.

**Also.** The cutoff control — §8.6a names it — was `border-zinc-400`, 2.56:1 against §4's 3:1 graphical bar, on the one control the report is steered with; it is `Field`/`Input`/`Button` now, which makes it 44px and 3:1 in one edit. `weeksSince` and the last spend are `.numeric`: they are the columns the eye runs down over thirty rows (§5.2). "No number on her record" lost its pronoun (A-091's fourth finding, one row it had not reached).

**And one the item did not go looking for: a pre-existing spec that fails on the clock, hiding a real gap.** `booking.spec.ts`'s A-071 case — "re-offers the same time with somebody else, rather than dead-ending her" — failed on `main` before a line of this item was written, and would have failed for anybody who ran the sweep that morning. It picks the first day and the first time offered; at 09:56 the first slot left today was **12:00**, which is the one slot of the day exactly ONE stylist can take, because the other three break 12:00–13:00 and a 45-minute Cut runs into it. Blocking her leaves nobody, the product refuses correctly, and the test was measuring the hour. Pinned to the second day — a whole open day, whose first slot is 09:00 and every qualified stylist is free at it — with the precondition written down, because the test's subject is *somebody else*, so a time only one person can take is not a fixture for it.

**What that exposed is A-097, and it is not a test problem.** When there genuinely is nobody else free at that instant, the flow falls back to the **vanished stylist's** day, which is empty by construction. The screen said *"No appointments available that day. Please choose another day."* on a Saturday with three stylists free from 13:00 and an empty book. She said NO PREFERENCE; the fallback is carrying a provider she never chose. Left as a row rather than fixed here — it is A-071's feature, not a design pass — and the spec no longer covers the path, which the row says.

**Left behind.** The cutoff control still scrolls away — 6.6 screens of list and the number that steers it is only on the first one; sticky chrome on a 768px tablet is a trade §8.6a does not make for us, so it stays a note. The count line says "23 clients, worth $4,180" and not how many have already been rung, which is the fact somebody returning to the list on Thursday actually wants; that is a change to what the screen SAYS and §9 puts it out of a design pass's scope. The gallery's thirty rows carry an empty payload so a press cannot write a mark (A-090's rule, A-091's precedent). `client-flag.tsx`, `client-picker.tsx` and `date-jump.tsx` still render raw zinc.
...
**THE FINDING WAS UNDER THE SHEET, NOT ON IT: the day view collapsed a segmented visit to its LAST block, so a two-hour colour was drawn at the wrong time, at a third of its height, with the hour the stylist spends applying it showing as empty column.** Measured on the seeded book, Dana's Wednesday, before anything was changed:

```
Tom Byrne, Colour, 09:00–11:00   chip drawn top:127.5px height:82.5px
                                 = 10:25, 55 minutes            (its SECOND block)
Marcy Dunn, Colour, 13:00–15:00  chip drawn at 14:25, 55 minutes
09:00–09:30 and 13:00–13:45      nothing in the column at all
```

`findBusyAppointments` returns **one row per worked block** keyed by the appointment's id — that is the whole mechanism of D-29/A-030, and its own header comment says so in capitals. `day-view.ts` joined it back with `new Map(busy.map((b) => [b.id, b]))`, which is last-wins. A colour therefore arrived as its develop-to-finish half.

**Why three demo checkpoints walked past it.** The collapse keeps the LAST block, so the range still *ends* in the right place. The gaps are subtracted from the busy spans rather than from this, so they stayed correct — the develop time was offered, the free time after the colour was offered, and the engine never reads this field at all. Only the START moved, and only on a **segmented** service, which no fixture in the suite had ever created and which is the salon's most valuable one. Every unit test, every e2e, every axe run and every walk was green over a grid that was answering "who is with a client at quarter past nine?" wrongly.

**And what actually found it was the paper**, because a table has to say with a SORT what the grid says with a position: the printed sheet put a 09:30 override **above** the 09:00 colour it overlaps. A stylist reads down that page and works the day in the order it is printed. The fix is four lines — min-start, max-end across an appointment's blocks — and the guard asserts BOTH edges, because an assertion on the end passes against the bug.

**The second: a list cannot say "at once", so the two rows that collide had to say it in words — and the marker §5.4.11 calls the most important visual in the product was on the tablet and on none of the eight pages the stylists hold.** Only a staff override can put two clients in one stylist's hour; D-8's zero-width blocked range is precisely what gets it past the exclusion constraint, and on the grid the two chips visibly overlap. In a table they are consecutive rows, which is the shape of *sequence*. The marker is on the paper now, and so is the reason somebody typed — the chip has no room for it, and "why are there two people at half past nine" is a question the paper cannot answer with a click.

**The third: the sheet's own comment calls it "who is coming", and it printed the one client who definitively is not.** The filter is `occupiesTime`, which deliberately keeps `no_show` (D-7) — her hour is still gone. She printed in the same ink as everybody else, with nothing to say she never came; and once the desk gave her remaining time back (A-069), whoever bought that time printed against the same hour with nothing to separate them. Every non-`booked` status now carries the chip's own word, read from the chip's own map rather than typed as a ninth copy of the status list.

**Which is one defect three times, so the guard is drawn at the shape rather than at the three.** `GridItem` has two renderers and only one of them ever gets told: A-069 added `released` and told the chip, A-090 added the status word and told the chip, §5.4.11's marker was drawn on the chip. `packages/design/sheet-parity.test.ts` asserts that every `item.<field>` the chip reads is either on the paper or written down with the reason it is not — and that no exemption names a field the chip no longer reads, because a rule pointing at nothing passes forever (A-092's lesson, one layer up). It fails on the day somebody adds a tenth field to the chip and does not think about a printer.

**The fourth, and it is print-specific: the file's own header comment swore the date is on every page, and it was not.** A browser repeats `<thead>` at the top of every printed page and repeats nothing else. The stylist's name and date lived in a `<header>` above the table, so page two of anyone with more than a page of clients arrived carrying "Time | Mins | Client | Space to write" and no name and no date on it. Proved on a real PDF of the seeded book: Priya's Wednesday is fourteen clients, it breaks after 11:40, and page two opened with the column headings alone. **It needs a stylist with a full day to exist at all, and every fixture in this spec seeded one appointment.** The identity is inside the `thead` now — a `<td>`, not a `<th colSpan={4}>`, which would be announced as a header on every cell below it — and the same PDF now opens page two with "Priya · Wednesday 2 September · 2026-09-02 · 09:00–17:00".

**Also.** A stylist who is OFF and a stylist with an empty day printed the same eight words; they are "Not working today." and "Nothing in the book." now, and the hours are in the page header, because on paper there is no shaded band to read them off and the difference between the two is a day the desk should be selling. The blank column was headed **"Notes"** while the notes printed in the client's cell beside the client they are about — it says "Space to write". `border-black` and `border-zinc-500` became `line-strong`/`line-control`: black-on-black is invisible on the dark tablet the desk *previews* the sheet on before spending the paper, and the print block already flattens every token to black. Times are `.numeric` rather than `font-mono` — a column of figures compared down a page is exactly what §5.2 reserves that for.

**Left behind.** The develop gap is drawn as an ordinary gap chip painted over the colour's chip — the same overlay idiom as A-069's released time — rather than as a hatched band inside it; it reads correctly and the brief's §5.4.2 wording ("gaps inside a service") would be better served by the second, which is a chip change and not a sheet one. `DaySheet` has no composition on `/staff/design`: it is a whole page with a real URL rather than a component that only ever appears one instance at a time inside a day, which is what the gallery exists for. The scribble column is one cell per row, so a page of ten-minute fringe trims gives the stylist fourteen small boxes rather than one big one — the brief asks for a blank column and does not say how tall. And per-row projected times are still not printed: D-22's stamp on the page header is what the sheet promises, and the row-level projection would be a promise the paper cannot keep.
...
- **Every screen failed axe in the dark scheme** — 12, 13, 4, 3, 3 and 4 nodes across the six states, all of one value: `text-zinc-500` at **4.1:1** on `#0a0a0a`, and **3.67:1** inside a selected card. Every duration and every price in the salon's catalogue, the step counter, "Back" on three screens, and on the last screen the price of her own appointment. `/book` had six green axe runs over its life and not one of them had ever seen this page, because Playwright's default colour scheme is light and no spec here had changed it.
- **"Back" was 31×20 CSS px**, on three of five screens, failing WCAG 2.2 SC 2.5.8's 24×24 on *both* axes. It is the control a thumb reaches for whenever the last tap was wrong. axe carries no target-size rule, which is exactly why six accessibility runs never mentioned it.
- **The two buttons the flow exists to reach were 36px tall** (`Continue` 88×36, `Confirm appointment` 162×36) and sat at their text width beside another control.
- **Twenty day options × 50px is ~1000px** of near-identical rows on a screen 844 tall.

**The fix is the tokens, not a new palette.** `/book` and `/manage/[token]` sit outside the `(site)` route group on purpose (A-062: the brand chrome above a booking flow gives every text locator two matches), so they were never on the marketing identity — they were raw zinc with hand-written `dark:` twins, which is what the staff app was before A-088. Retiring them onto A-088's tokens fixes all 39 contrast nodes at once, because `--ink-muted` is zinc-600 for precisely this reason and the `var()` indirection flips it with the scheme by itself. §9's "the public palette stays as it is" is honoured: the bone/clay identity is untouched, and this item invented no colour.

**Composition, in four places.** The day list is two columns with the weekday stacked over the date — ~640px instead of 1000, and the weekday leads because that is what she chooses by ("a Saturday"); the date is how she confirms it, not how she finds it. The time list is a grid of equal columns rather than `flex-wrap`, which had been sizing every button to its own label and leaving a ragged half-empty last row — three columns also leave room for the FB-5 qualifier (`01:30 CDT`) without the label wrapping inside the button. Both primary actions are `Button` at its 44px size, full-width until `sm`, and on the details screen the Back link is *beneath* the confirm button rather than beside it, so a thumb cannot take the wrong one. `BackButton` is `Button variant="quiet"` — same visual weight, 44px.

**`readableDayParts` is beside `readableDay`, and `readableDay` is built out of it.** The stacked day needs the weekday and the date as two strings; splitting the formatted label at the call site would have been a second, weaker copy of the formatter that breaks silently the day the format changes, and deriving a weekday in a client component is the banned `new Date(string)` wearing a hat (D-3). One function returns the parts, the other joins them, and the e2e asserts the day she taps and the day the next screen names are the same day — the "two answers to one question must be equal" rule (A-082, A-069) applied to a label instead of a predicate.

**The latency defect, which no measurement finds.** Continue, a stylist and a day each fire a server action, and the screen said nothing at all while it ran. On a phone at night that is a tap that did not work, so she taps again and a second `listTimesOn` goes out. Each step's controls now sit inside `<fieldset disabled={pending}>` — one native attribute that reaches every control under it, Back included — with `aria-busy` beside it. The spec makes it deterministic by holding the round-trip open with `page.route` rather than hoping the machine is slow, which is the same reflex as the barrier-based race tests.

**A silent accessibility bug the primitive's own header had predicted.** The form rendered its error `<p>` only once there *was* an error. An `aria-live` region has to exist before its text lands in it, so every server-side refusal on this flow appeared on screen and announced nothing. A-089 wrote that rule into `Field`'s header comment and this form was the first caller that needed it: `error={result?.fieldErrors?.name ?? ''}` renders the region always, empty at rest.

### And the thing the mutation found: A-088's type scale has never been applied anywhere a colour rode along

**The assertion that could not fail.** The spec asserts the form's inputs are ≥16px, because iOS Safari — the client's phone and the salon's iPad equally — zooms the viewport when a focused input's font-size is under 16, leaving the page magnified and scrolled with the submit button off the right edge. `Input` shipped at `text-body`, which A-088 measured this product to be: 14px. Putting the 14px back **did not make the test fail**.

**`tailwind-merge` files every `text-*` it does not recognise as a text COLOUR.** It resolves conflicts from a built-in map of Tailwind's own class names. It knows `text-sm` and `text-base`; it has never heard of `text-body`, `text-caption`, `text-section`, `text-page-title` or `text-display`, so it drops each one as superseded the moment a real colour follows in the same merge:

```
twMerge('text-body text-ink-primary')  →  'text-ink-primary'
```

Which is every primitive A-089 built. `Button`'s size string carries `text-body` and all four variant strings end in a `text-ink-*`, so **no Button in this product has ever rendered at 14px**; `Field`'s label loses `text-caption` to `text-ink-secondary`; `EmptyState` and `Tab` the same way. They rendered at the inherited 16px — a plausible size. Nothing looked broken, nothing failed, the compiler saw two valid class names, `primitives.test.ts` reads the *source* where `text-body` is present and correct, and axe measures contrast rather than size. **The five roles A-088 audited this product into were applied nowhere a colour rode along, for five items.**
...
**THE FINDING WAS THE SIZE OF IT: 275 colour-contrast nodes across twelve staff routes, and every single one of them the same value.** `text-zinc-500` is `#71717b`, which is **4.1:1 on `#0a0a0a`** against AA's 4.5:1 — 158 nodes at 14px, 114 at 12px, 2 at 16px, 1 bold. Measured route by route on the seeded book: 194 on `/staff/clients/[id]`, 27 on `/staff/services`, 23 on `/staff/availability`, 12 on `/staff/settings`, 4 each on the dashboard and the waitlist, 2 on the appointment detail and the room, 1 each on call-down, conflicts, providers, unfinished, and 3 on `/staff/book`. **Light was clean on all twelve.** That is the whole mechanism of the blind spot: Playwright's default colour scheme is light, so a palette built to flip with `prefers-color-scheme` had exactly half of itself measured, and the failing half was the one nobody looked at. Forty green axe runs and none of them could say a word about it.

**The fix for the colour was one substitution. The fix for the BLINDNESS is the helper**, and that is the half that lasts. A-092 and A-094 had each found this same defect on their own screen and each fixed it *on their own screen* — a light-only assertion is not a bug in one spec, it is the default shape of the assertion, so patching the three specs that happened to notice leaves the other thirty-eight writing it the same way tomorrow. The scheme loop now lives inside the helper where a call site cannot skip it, and the lint rule closes the only door around it. Verified by writing a spec that imports `AxeBuilder` directly and watching lint refuse it.

**WHY THE HELPER KILLS TRANSITIONS INSTEAD OF RELOADING, which is what the three existing dual-scheme specs did.** Flipping the media query on a live page catches every control mid-`transition-colors`, and axe samples the blend — A-092 measured 583 nodes of colours nobody ships. A reload fixes that and also throws away everything the test had built: an open panel, a typed form, a five-step booking flow, the two call marks `dashboard.spec.ts` clicks onto the lapsed list before measuring it. Measured on four staff routes, both directions: the naive flip invents **10** phantom nodes on the client record and **2** on the day grid, while freezing transitions and reloading agree **exactly**, every time. So the helper takes the one that keeps the page — and `dashboard.spec.ts` now asserts its two marks are on screen **once**, rather than once per scheme, because they survive the dark pass.

**THE PROCESS FINDING, and it happened to this item's own measurement.** The first sweep reported **46 routes clean in both schemes** and was completely worthless: `waitForURL(/\/staff/)` matched `/staff/login` itself, so the sign-in click never took, and axe measured the login page forty-six times over. It is the identical shape of the defect the item exists to fix — *a green accessibility run over a page that is not the page you think you measured* — and the only thing that caught it was printing `page.url()` beside each result. **An axe assertion is worth what its fixture is worth**, which is why this repo's rule is to assert the interesting content is on the screen before the scan; the version of that rule for a sweep is to assert you are on the route you named.

**A green run can also mean the class does not exist.** `text-ink-muted` had to be checked for real, not inferred from the violations vanishing: had Tailwind not generated it, the text would have inherited `--foreground` (`#ededed` in dark) and passed contrast *by rendering muted text as primary text*. Confirmed against the built stylesheet — `.text-ink-muted{color:var(--ink-muted)}`, resolving `#52525b` light, `#a1a1aa` dark, `#000` print.

**The type-scale pass A-094 left behind.** A-094's `cn` fix means every `Button`, `Field` label, `EmptyState` and `Tab` in the staff app went from the inherited 16px to A-088's 14/12px, and nobody had looked at those screens since. Walked here in both schemes at 1024×768 on the seeded book — day grid, client record, appointment detail, waitlist, `/staff/opened`, services, settings, lapsed. No regression: the scale reads as the dense operational screen A-088 measured this product into, and the 44px desk target is unaffected because `Button`'s `md` size floors on `min-h-11`, which does not depend on the font size.

**Left behind.** **`/staff/dashboard/overruled` has no e2e spec at all** — not an axe one, not any. Its `text-zinc-500` was fixed with the rest, but nothing guards it. Deliberately not papered over with an axe run here: the route renders an empty list without `from`/`to`, and a scan of an empty list is a scan of the chrome — the fixture needs a genuinely overruled cancellation, which is an item, not a line. **`/staff/design` returns 50 axe `incomplete` results in both schemes** (41 `aria-prohibited-attr`, 5 `form-field-multiple-labels`, 2 `duplicate-id-aria`, 2 `color-contrast`) — `incomplete` is not `violation` and no assertion covers it; it is gallery-only, on a page that renders the same primitive many times over, and it appears on no real staff route. And `expectNoAxeViolations`'s `impacts` option is a way for a screen to stop being measured: four screens pass `SERIOUS` today, inherited from the assertions they replaced. It does not weaken this item's sweep — axe rates `color-contrast` **serious** — but it is the loosest thing in the file.
...
**THE DEFECT WAS ONE ARGUMENT, AND IT WAS THE ARGUMENT SHE NEVER SUPPLIED.** A-071's promise is that a client who said "no preference" is never sent back to the time list: lose the race for the stylist the flow picked for her, and the same time is re-offered with somebody else. `anyProviderAt` returns `null` when nobody qualified is free at that instant, and the flow then fell through to "here are the other times still free" — computed as `listTimesOn(serviceIds, input.providerId, day)`. **On this path `input.providerId` is a stylist SVC-02 assigned; she never chose her, and never heard her name until the details screen.** Worse, the specific reason `instead` came back null is usually that this person has GONE — so the fallback offers one absent stylist's day, which is empty by construction. Proved on 2026-09-05 at 09:56: the flow picked Tess at 12:00, Tess was taken out from under the page, and the screen answered *"No appointments available that day. Please choose another day."* on a Saturday with three stylists free from 13:00 and an empty book.

**Half of A-071 was already right, which is why nothing caught it.** The `instead` arm asks across everybody; only the fall-through did not. Two arms of one `catch`, forty lines apart, answering the same operational question two different ways — the shape this repo keeps finding (A-075's two status lists inside one function, A-069's chair envelope and chair body). The compiler cannot see it: both arms return `OfferedTime[]`, and the wrong one is *populated and plausible* on any day the vanished stylist is merely busy rather than absent.

**THE FIXTURE IS THE ITEM, and it needs a room where the two answers differ.** Any book where everyone works the same hours gives `listTimesOn` and `listAnyProviderTimes` the same day, so the defect is invisible — which is exactly why the pre-existing A-071 spec, pinned by A-092 to "a whole open day so its first slot is 09:00 and every qualified stylist is free at it", could never reach it. The new spec builds the opposite: the three seniors start at ten on the next Tuesday, so **09:00 is Tess's and only Tess's** and every time from 10:00 is theirs. Tess then goes off sick mid-form. The two questions now answer *nothing* and *seven hours*, and the test asserts the precondition (`with Tess` on the details heading) rather than assuming it — if the fixture ever stops producing a one-stylist instant, the test fails saying so instead of passing for the wrong reason. Verified red before green, in both directions: with the fix reverted it fails on `No appointments left that day.` resolving to one element.

**The second half was the confirmation screen, and it is the same reader written twice.** A-071 changed the details heading to `time.providerName ?? provider.name` and left the `done` screen on `provider.name` — which on this path is the sentinel's label. Every client who ever booked through "No preference" was told *"Cut with No preference"* on the one screen she keeps. Nothing asserted on it: the A-056 spec checks the "Your appointment is confirmed" heading and stops. Now asserted by name.

**WHAT THIS ITEM'S GATE FOUND ABOUT THE MACHINE, WHICH IS NOT ABOUT THIS REPO.** `density-seed.test.ts` failed three consecutive runs in two different places — once the `beforeAll` hook at 120s, twice a re-seeding test — and the seed measured **117.6s** against a `SEED_TIMEOUT` written for "~100 appointments, around four seconds". The obvious reading was that A-095's second column had outgrown the budget, and raising it was one line. **The obvious reading was wrong.** `PROGRESS.md` records A-095's seed at ~15–18s, and `pg_stat_activity` showed eleven idle connections against `countertop_test` — a *different project* mid-sweep, with its own `next-server` and chrome-headless and a load average of 28.9. A wall of timeouts across one file is an environment symptom, and the tell was in the conventions before it was in the log. Re-measured on a quiet machine: **76.9s for the whole file, which seeds three times**, against 117.6s under the neighbour — so the 120s hook has real headroom and **the budget stayed where it was**. Raising it would have hidden the next genuine regression behind a bigger number. `PROGRESS.md`'s A-095 note ("~15–18s") is still the stale figure it was before this item; nobody has re-measured the seed alone.

**Left behind.** `sameTimeWithSomebodyElse` still asks the room's question anonymously (`holderKey` unset) at a point where `confirmAppointment` has already resolved the client five lines above — the strict direction, so it can only offer *fewer* times than the write would accept, but it is the A-083 shape and the public flow is the caller A-083 did not reach. And the fall-through's wording (*"that time has just been taken"*) now covers a case where nothing was taken at all: nobody is free at that instant. It claims no cause about the salon, which is what D-10 requires, but it is a sentence about a race that did not happen.
...
**THE WALK WAS CLEAN WHERE IT HAS NEVER BEEN ABLE TO BE.** A-095 is the item that made this checkpoint worth running, and it delivered exactly what its row claimed. Checkpoint 7 wrote down two scenes as unwalkable; both were walked:

```
                        checkpoint 7            checkpoint 8
future columns          Dana + Priya only       all four, on all 8 days
waitlist / call marks   0 / 0                   3 / 2
no-resource-free        0 in 21,184 offers      44
```

So checkpoint 6's property — *a slot on the screen is a slot the write path will accept* — was re-run for the first time on a room that genuinely binds, rather than one where the chair axis was a shadow of the provider axis: **1,609 offers, 0 without a chair**, every offered slot put to `findFreeResource`, the chooser the write itself uses. And A-096 holds everywhere: **50 route×scheme pairs, all 200, zero axe violations in either scheme, zero console errors.** The freed-slot loop walked end to end — `/staff/opened` → the waitlist panel with the instant in the URL (D-4) → Book — which no checkpoint has ever been able to do.

**WHAT THE WALK FOUND WAS ONE SHAPE, THREE TIMES: a screen that answers a question nobody asked, in the reassuring direction.** `/staff/dashboard` renders **0.0% utilization beside its own count of 157 bookings**; `/staff/dashboard/overruled` says *"Pick a week"* and *"None that week"* in consecutive lines; `/staff/book` says *"That stylist is not on today"* when you named no stylist. An empty state that says nothing is interrogated. An empty state that *answers* is believed — and all three answer in the direction that stops the enquiry: the salon is empty, nobody was let off, she is not in.

**The utilization one is not a bug and that is what makes it a finding.** RPT-02 freezes the formula (`Σ{completed, no_show} ÷ Σ(working − breaks − time off)`), `dashboard.ts` implements it exactly, and `dashboard.test.ts:95`/`:101` assert it on purpose. The denominator is non-zero, so `0.0%` is spec-correct. **What was never specified is which week the tile is shown for** — a retrospective formula on a dashboard that opens on the current week reads 0.0% for every stylist on every week that has not happened, and it sits directly above *"who to ring to fill a quiet Tuesday"*, an action about the future. Invisible because the only week ever asserted is DEMO_WEEK, a **fixed past week** — the one kind where the retrospective answer is right. Even that week reads 0.0% for three of four stylists; the constant test asserts Dana and looks no further. Written as A-101 with the formula explicitly out of scope: it needs a decision, not an edit.

**The `overruled` one is A-087's own fix, un-applied to the room next door.** `dashboard/appointments/page.tsx:64` reads `{fromDay && toDay ? 'Nothing matches this filter.' : 'Pick a week from the dashboard.'}`; `dashboard/overruled/page.tsx:55` is unconditional. A-087 repaired the screen it was standing on. **That is A-096's rule verbatim — patching the rooms that noticed leaves the door open** — and the room that did not notice is the one with no e2e spec. Audited by shape rather than by screen: of the four parameter-driven zero-row states, two are right and two are wrong, which is what makes A-104 a bounded row instead of a two-line patch.

**THE OPERATOR FOUND THE ONE NO WALK REACHES, AND IT IS THE EXPENSIVE ONE.** Untick a stylist — the single action the product offers for "she has left" — and her **entire forward book leaves every screen in the salon** while the clients keep getting reminders. Proved by flipping one boolean on `db:reset:test`: **106 future appointments worth $2,870 gone from the grid and the printed sheet, 106 chairs still held, `/staff/conflicts` reporting 0 stranded.** Four readers filter her out, one line each, **each correct in isolation and two of them carrying a comment explaining why**; `reminders.ts` correctly has no provider filter, so she is told to come, taps the manage link, and the engine offers her an empty day forever (`candidatesConsidered: 0`). All four citations were verified against the source before the row was written. **The right answer is already in this codebase one axis over**: `room.ts:116` keeps a retired *chair* rendering until its last hold ends, and A-046's dialog says so out loud. A retired stylist must stay visible, closeable, conflictable and sellable — she must only stop being **bookable**.

**A-099 is a written promise the product does not keep.** D-8's last clause is *"the day view renders the true collision"*. It does not: both chips of a knowing double-book compute `top=60 height=55`, `CHIP_SHELL` is `absolute inset-x-1` for every chip in the product, and there is no lane, offset or width anywhere in the day surfaces — so **the client already in the book is the one who disappears**, under the chip wearing the override marker. The desk overrides *because* it intends to see both.

**A MEASUREMENT THE WALK OWED, AND A CORRECTION IT OWES ITSELF.** The seed alone is **16.8 s**, so A-095's "~15–18 s" was accurate all along and A-097's 76.9 s for a file that seeds three times reconciles against it — **the 120 s hook budget needs no change**, and PROGRESS no longer carries an unmeasured number. And the walk's first scripted pass reported the `/staff/opened` CTA bouncing back to itself, which would have been a dead sell-the-slot button and the most valuable finding of the day. **It was the script** — a `click()` whose `waitForLoadState` resolved before the navigation. A direct GET returns 200 with the panel rendered. A checkpoint's own instrument can manufacture a defect, and the rental build's lesson applies to this build too: *prove the assertion before reporting the defect.*

**Left behind.** The demo book runs **eight working days forward** and then nothing until the fixed fall-back day on 1 November, so any horizon-related surface finds nothing past 16 September. **The day grid on a closed day renders a full midnight-to-midnight axis** where an open day renders 09:00–17:00 — cosmetic, but it is the first screen after sign-in on two days in seven for a salon closed Sundays and Mondays. `/staff/design` returns **53** axe `incomplete` results in both schemes, up from A-096's 50, still not violations and still gallery-only. The "Book with Dana" button beside "off today" was checked and is **not** a dead end — it opens the ordinary panel with a day picker — and is recorded here so the next reader does not re-investigate it.
...
**THE DEFECT WAS NEVER IN ANY ONE READER, WHICH IS WHY FORTY GREEN RUNS COULD NOT SEE IT.** Each of the five filters was locally defensible and two carried comments explaining themselves:

```
day-view.ts:144    active: true                → removed the COLUMN rather than emptying it
impact.ts:361      conflicts derive from hours + absences; deactivation writes NEITHER
unfinished.ts:94   provider: { is: { active }}  → her visits could never be closed out
opened.ts:217,340  provider: { is: { active }}  → a Saturday she frees never reached the screen
dashboard.ts:63    active: true                → she left the week she actually worked
```

Plus a sixth found by the grep the row asked for and not named in it: `waitlist/page.tsx` resolved the freed-slot provider against an active-only roster, so naming a departed stylist made `freedSlotFrom` return `null` and **the entire "who wants this slot?" panel vanished with no error** — the page rendered as though it had been reached with no parameters at all. It also printed `?` instead of her name against every waiting client who had asked for her.

**The answer was already in this codebase one axis over.** `room.ts:116` is `.filter((r) => r.active || r.holds.length > 0)` and A-046's dialog says it out loud: *they keep the chair until they are done, and nothing new is seated here.* The provider query is now that predicate written in SQL — `{ OR: [{ active: true }, { appointments: { some: <the window this screen covers> } }] }` — and the two surfaces that need it use the window they actually cover (instants for the grid, `startDay` bounds for the week's report). The other three needed no predicate at all: the appointment set already bounds them, so the filter was simply deleted.

**THE HALF THAT KEEPS THE FIX HONEST IS THE GAP.** Deactivation writes `Provider.active` and nothing else — not her hours, not an absence — so making her column render would have drawn every free minute between her remaining appointments as a bookable gap chip carrying `/staff/book?provider=…`, which `slot-query.ts` refuses. That is the offered-then-refused class this repo has now caught four times, and it would have been *introduced by the fix for the invisibility*. `loadColumn` returns `gaps: []` for an inactive provider, at the source, so the grid, the sheet and anything else reading `gaps` follow without knowing. The header's "Book with —" link and the running-late/push controls go with them, and the header says **off the roster** with a link to `/staff/conflicts`, which is the screen that does something about it.

**Where the freed time goes.** `/staff/opened` now carries her spans with `providerActive: false` on the row, and the matcher's Book button reads `provider=any` rather than her id — the same hour of the same Saturday, with whoever is free. The week a stylist leaves is the week her book gets cancelled, so this screen was going quiet at precisely the moment it had the most to sell.

**What the tests are.** One new file, `packages/db/settings/off-roster.test.ts`, and it is deliberately one file rather than six additions to six suites: a test living inside any single suite asks that suite's question and passes, which is exactly what happened. One operational fact — *Tess has left and her clients are still booked* — asserted against every reader in the same `it`, on a fixture where the departed stylist and the remaining one are **different**, plus the paired assertion that she is still unbookable and that Dana beside her still has gaps (so it cannot pass against a grid that has stopped offering anything). `apps/web/e2e/off-roster.spec.ts` walks it through the real Deactivate button, past A-041's refusal, because the two halves only meet on screen.

**THREE EXISTING TESTS ASSERTED THE DEFECT BY NAME AND A FOURTH PASSED FOR THE WRONG REASON.** `opened.test.ts`, `opened-vacated.test.ts` and `unfinished.test.ts` each had a green `it('drops … a provider who has since left')`; all three are reversed with the reasoning written in. The fourth is the warning: `day-view.test.ts`'s `'drops a deactivated provider'` **passed before and after the fix**, because it deactivated a provider who had nothing booked that day — so it was asserting "an empty column is not drawn" under a name that claimed something far stronger. It is now split in two, and the new half books a client first.

**A FIXTURE TRAP THIS COST A GATE RUN, now in CLAUDE.md.** The e2e spec wrote its appointments by hand and set `blockedStart`/`blockedEnd` directly — which a trigger overwrites, deriving them from the row's own `bufferBeforeMinutes`/`bufferAfterMinutes`. Those default to 0, so the envelope was stored equal to the body: a row the product cannot produce. The span it later freed was then exactly one Cut long, and `matchFreedSlot` measures the whole footprint (buffers included), so **the matcher found nobody and said so in a normal sentence** — no error, no stack trace, just "Nobody on the waitlist fits this one." The first version of that assertion also passed against the wrong thing: it checked the waiting client's name at PAGE level, and she is in the standing queue at the bottom of the same screen, so it was green one line above the assertion that actually failed. It is scoped to the matcher panel now.

**Left behind.** `reminders.ts` still has no provider filter and is still right to: a reminder is about an appointment. But the client who receives one for a departed stylist and taps the manage link is offered an empty day, because the engine correctly refuses her — the harm chain now breaks one step earlier (the desk sees the stranded list and deals with it) rather than at the link itself. `/staff/availability` remains active-only, so a departed stylist's hours cannot be edited; that is deliberate and the resolution lives on `/staff/conflicts`. And a provider whose only remaining work sits in the ±24h slack around a day — but not on the day itself or in her windows — renders an empty "off the roster" column; `room.ts` has the same asymmetry for a chair, and it is honest rather than wrong.
...
**THE PROMISE THAT WAS NOT KEPT.** D-8's last clause is a sentence about a screen: an override writes a zero-width blocked range plus `overriddenFromRange` *"so the constraint never lies and the day view renders the true collision."* It did not render it. Two clients, same stylist, same instant, the second booked through BOOK-05 with a typed reason — both chips computed the same `top` and the same `minutes`, and the horizontal extent was not per-item at all:

```
appointment-chip.tsx:89   CHIP_SHELL = 'absolute inset-x-1 …'   ← every chip in the product
view-model.ts:23          GridItem { top, minutes }             ← no lane, no offset, no width
```

So the later chip in DOM order painted over the earlier one, opaque, `overflow-hidden`. **The client who was already in the book is the one who disappeared** — and the chip left standing wore the override marker, which reads as one deliberate override rather than as two people at ten. The desk overrides *in order to see both*.

**ONE FACT, FOUR READERS, AND ONLY ONE OF THEM HAS GEOMETRY.** `GridColumn.items` is read by the day grid, by `DaySheet` (the paper), by `ProviderDay` (the stylist's phone) and — through `RoomTrack` — by the room strip. The grid loses a client behind a z-order; the sheet and the phone list have no z-order at all and lose her a different way, by putting the pair on **consecutive rows**, which is the shape of a queue. `day-sheet.tsx`'s own header said so at A-093 and reached for the override marker as the answer; the marker says *somebody decided this*, not *these two are at once*. So the wording is computed once in `lanes.ts` and every reader prints the same sentence, the accessible name included.

**THE ROOM STRIP HAD THE SAME BUG AND NOBODY WAS LOOKING AT IT.** A-063 permits two envelopes to overlap on one chair for the same holder (bodies never do), so a client's back-to-back visits legitimately collide there — same `absolute inset-x-1`, same last-wins paint. It goes through the same `assignLanes`, which is the reason the packing lives in its own module rather than as a private helper inside the view model: a second opinion about "at once" is how this repo has produced most of its defects.

**Half-open, and it is load-bearing.** 10:00–11:00 and 11:00–12:00 are consecutive clients, not two people in one chair; a lane predicate written with `<=` halves the width of every back-to-back column in the salon. Lanes are per overlapping **cluster**, never per column, so one double-booked hour does not halve the day around it.

**What the tests are.** `apps/web/lib/day/lanes.test.ts` — eleven cases, each about a *different* wrong answer that is invisible on a simple book: back-to-back halved, the whole column halved, a gap chip laned (it is drawn over a colour on purpose, A-030), a lane not reused once its occupant has finished, and `concurrent` naming the whole cluster rather than only who actually overlaps. Running it needed one line in `vitest.config.ts`: the unit suite was `packages/**` only, and the alternative was a second copy of the algorithm in a package or no unit test at all. The e2e block in `day-grid.spec.ts` asserts **both** chips — `toBeVisible()` on the override alone passes against the defect, because the override is the chip that survived it — then that their boxes do not overlap horizontally, then that the lone 09:00 keeps the full column. The gallery gets a fifth composition (`A_DOUBLE_BOOKED_HOUR`), and its fixtures go through `withLanes` rather than hand-typing lane numbers, so `/staff/design` cannot draw a pair the product would draw differently.
