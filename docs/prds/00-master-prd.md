...
- **SVC-01** Services have name, base duration (minutes), `bufferBeforeMinutes` (default 0), `bufferAfterMinutes`, price, active flag, assigned providers.
- **SVC-02** The provider×service assignment carries `durationOverrideMinutes?`, `priceOverrideCents?`. Slot computation uses the provider-specific duration; "any provider" search computes per provider. An unassigned provider never appears in that service's booking flow.
...
- **CLIENT-01** `Client` keyed on normalized phone; email optional; no login. Staff merge duplicates (history follows the merge).
...
`Business` (timezone, hours pattern, policies: slot interval, lead time, cutoff, no-show block threshold) · `Provider` · `Service` · `ServiceProvider` (qualification + overrides) · `ServiceSegment` (schema affordance only in v1 — one ACTIVE segment per service; D-12) · `ResourceType` / `Resource` (affordance only; D-12) · `WeeklyWindow` (+ `WindowBreak`) · `DateOverride` (+ child windows) · `TimeOff` / `AdHocBlock` (instant intervals) · `Client` · `Appointment` (+ `AppointmentServiceLine`, ordered; v1 flows create one line, VISIT work makes it plural) · `AppointmentEvent` (append-only) · `ManageToken` · `NotificationOutbox` · `WaitlistEntry` · `StaffUser` (single credential v1).
