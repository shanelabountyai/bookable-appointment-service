...
### SEG — Segmented durations (Phase 3, D-12)

- **SEG-01** A service's body duration is an **ordered list of segments**, each `{ordinal, durationMinutes, isGap}`. A simple cut is one segment; a colour is three (45 active / 35 gap / 30 active). `isGap` marks a segment the **client** occupies but the **provider** does not — colour developing, a perm setting. Buffers stay on the *service*, not the segment: they are the turnaround either side of the whole visit, not between its parts (same reasoning VISIT-01 uses to refuse stacking buffers between lines). `Service.durationMinutes` remains the authoritative footprint and MUST equal the sum of the service's active segments — a single-segment service is byte-identical to a v1 service, which is what makes this additive rather than a rewrite.
- **SEG-02** A provider's `durationOverrideMinutes` (SVC-02) scales the **active** segments only, integer minutes, remainder to the last active segment. **A gap never scales**: colour develops for 35 minutes regardless of who mixed it, so applying a stylist's speed to chemistry is factually wrong and would silently mis-time every segmented booking. An override that would drive any active segment to ≤ 0 is refused at save time, not at booking time.
- **SEG-03** The day grid and the appointment detail render a segmented appointment's gap as **visually distinct from its active stretches**, and the gap's length is legible. This is the whole feature's value before any engine change: the desk can see the genuine 35 free minutes at 10:45 and book them by hand.
